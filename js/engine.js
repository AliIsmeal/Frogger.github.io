...
function timerFunc() {
  ...
  if (totalMin === 2) {
    document.getElementById("gameOver-model").style.display = "block";
    clearTimeout(myTimer);
...
let Engine = (global => {
  ...
  let main = () => {
    ...
    if (score === 13) {
      document.getElementById("congrats-model").style.display = "block";
      score === 0;
      clearTimeout(myTimer);
    }
  ...
  let updateEntities = dt => {
    let speed;
    allEnemies.forEach(enemy => {
      speed = (150 * dt) * Math.floor(Math.random() * 10 / 4);
      enemy.update(speed);
    });
    player.update();
  }
